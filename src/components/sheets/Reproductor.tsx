...
import { Minus, Pause, Play, Plus, Repeat, Square } from "lucide-react";

import type { Elemento } from "@/lib/melodia";
import { TEMPO_MAXIMO, TEMPO_MINIMO, tempoValido, type Momento } from "@/lib/reproduccion";
import { Reproductor as Motor, type Ajustes, type Aviso } from "@/lib/reproductor";
import { INSTRUMENTOS, guardarInstrumento, useInstrumento, type Instrumento } from "@/lib/sonido";
...
export default function Reproductor({
  ...
  const [aviso, setAviso] = useState<Aviso>({ estado: "parado", momento: null, progreso: 0 });
  ...
  useEffect(() => {
    let ultimo: Aviso = { estado: "parado", momento: null, progreso: 0 };
    ...
    const m = new Motor((a) => {
      ...
      if (cambioNota || a.estado !== ultimo.estado || Math.abs(a.progreso - ultimo.progreso) > 0.005) {
        ultimo = a;
        setAviso(a);
      }
    });
  ...
  const ajustes: Ajustes = useMemo(
    () => ({ tramos, compas, tono, tempo, instrumento, metronomo, repetir, semitonos }),
    [tramos, compas, tono, tempo, instrumento, metronomo, repetir, semitonos]
  );
